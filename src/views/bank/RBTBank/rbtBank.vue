  ...
  methods: {
    ...
    mortgageMarket(){
      this.$store.dispatch("RBT/approve",{
        spender:getContract.getContractAddress("RainbowBank"),
        amount:100000000000
      }).then(()=>{
    ...
    async getDeposit() {
      ...
      await this.$store.dispatch("RbtDeposit721/totalSupply").then(async length => {
        ...
        for (let i = 0;i<length;i++){
          let tempObj = {}
          tempObj.tokenId = i
          await this.$store.dispatch("RbtDeposit721/ownerOf", {_tokenId:i+1}).then(async res => {
            tempObj.ownerOf = res
            if(res.toUpperCase() == this.account.toUpperCase()){
              await this.$store.dispatch("RbtDeposit721/amount", {tokenId:i+1}).then(res => {
                tempObj.amount = res
              })

              await this.$store.dispatch("RbtDeposit721/tokenMetadata", {_tokenId:i+1}).then(res => {
    ...
    async initData() {
      ...
      this.$store.dispatch('RBT/balanceOf', {account:this.$store.state.app.account}).then(res => {
      
        this.userAmount = res
      })
      this.$store.dispatch("RbtDeposit721/balanceOf", {_owner:this.$store.state.app.account}).then(res => {
        this.amount721 = res
      })

      this.$store.dispatch("RbtDeposit721/tokenMetadata", {_tokenId:1}).then(res => {
